\chapter{Design Document - Concept for an Exergame for Elderly}
\label{chap:concept}
...
This chapter will primarily be a design document, as described in Chapter \ref{sec:designphase}. Here we present the concept, with its requirements, its story, and the elements that needs to be included in the game. We will also provide medium-fidelity prototypes for some scenarios to show "sketches" of the  This document can serve as guidelines for the development team to understand how the game shall function so that they see what they need to create. 
...
\centering
    \begin{tabular}{|l|l|}
       ...
       \emph{Interactive objects:} & The player character, rowboat with oars, \\ &  certain rocks lying in the middle \\ &  of the trail, logs over a creek, red hearts at \\ &  random places in the field, bought \\ \hline
	   \emph{Scripting objects} & the player character: will move to the \\ &  movements received as player input\\ \hline
	     & rowboat with oars: will move forward as the player \\ &  uses his/her arms with the oars.\\ \hline
	       & certain rocks lying in the middle of the trail: \\ &  If hit, the rocks will flash red and the player \\ &  will lose 1 point. \\ \hline
	        & logs over creek: If the player is in touch with \\ & the creek, he or she  will lose 1 point. \\ \hline
	         &  red hearts: If the player reaches for the heart, \\ &  he or she gets 1 point. \\ \hline
	          &  bought: If hit, the player will lose 1 point. \\ \hline
    \end{tabular}
...
\emph{Characters} \\
In this game the player character will be the only one included. As described in Chapter \ref{subsub:fictionalworld} the player character can be described by three different categories: avatars, actors and roleplaying. From the definition the player character here will be an actor. However, in commercial Kinect games the player character is called an avatar. Therefore, we do the same. The avatars are seen in third person. They do not have a distinct personality except from being a "a normal human being", but they are well integrated into the story. There are two different men and two women. All characters are dressed in training outfits, the women in training leggings and t-shirt, and the man in long pants and t-shirt. The different characters will have different colors on their t-shirt, showing which player they are. The men has either blue or green t-shirts, while the women has either yellow or pink t-shirts. The men have short hair, while the women have half-long hair. The characters will be described through a meaningful name. The names will international names, like Anne, Eva, Charles and David. 

\subsubsection{Mechanics} 

Progression game. Solve a story. Completes each chapter or level by solving puzzles. Player has to finish different chapters to proceed to the next level. The levels are building on each other. There is a climax at the end of each chapter. 

\emph{Quests:} \\
\begin{itemize}
\item Walk the trail. The higher the player lifts his or her legs the faster he or she walks.
\item Gather red hearts to gain points. 
\item Watch out for the rocks lying in the middle of the trail and walk pass them. This requires side-steps. 
\item Watch out for the bought hanging from the trees over the trail. This requires squats while walking.
\item Balance over the log that lies over the creek. This requires walking with both arms out. 
\item Log lying in the middle of the trail. Requires lunges. 
\item "Branching": Trail splits, and there are two possibilities: 1. Walk one trail with rock obstacles but a lots of hearts to gather. This will require some more time, but the player can gain more points by gathering hearts. 2. Walk the other trail which is without obstacles but no hearts to gather. 
\item "Branching": Trail splits, and there are two possibilities: 1. Keep walking the trail. 2. Row a boat over to the other side. Rowing requires legs wide apart and arms stretched out in front of the player and dragged back to their body (as actual rowing). 
\item When rowing watch out for the water lilies. This require moving the body to the one side to steer away from the lilies. 
\item Find all yellow question sheets. 
\item Answer all questions. There are different categories. Which category and which question the player gets is random. The game will remember which question has been before, so that the same player does not get the same questions. The level of difficulty will increase after the player has played the game several times. 
\item Solve puzzles. Different puzzles each time, for example link right flag to right country on the map.
\end{itemize}
...
\centering
    \begin{tabular}{|l|l|}
       ...
       \emph{Sound effects} &  when hearts are gathered there is a cheerful \\ &  "pling"-sound  \\ \hline
	    &  when taking the question sheet there is a \\ & "swosj"-sound.\\ \hline
	     & when the character is walking there is the \\ & sound of steps on the ground. Different sounds \\ &  for different surface, like walking on the rocks, the \\ &  earth or the logs.\\ \hline
	       & the sound of shoved water when rowing. \\ \hline
	       \emph{Ambient effects} & birdsong \\ \hline
	         & wind \\ \hline
...
\centering
    \begin{tabular}{|l|l|}
	   ...
	   \emph{Scripting objects:} &  apples on the tree: Will appear on the tree with a \\ & yellow-green color showing they are immature. \\ & After a while they will start to get red, and \\ & if the apples does not get picked while it  is red,\\ & it will become brown and fall to the grown. \\ & If the player picks the apple in time he or she will \\ & get 1 point. If not he or she will lose 1 point.\\ \hline
	     & baskets: The baskets will get filled with apples. \\ & The player gets 1 point if he or she manage to put \\ & the apple into the right basket. The basket is flashing \\ &when it is its turn  to get an apple. If the player \\ & puts the apple in the wrong basket, he or she will not \\ & get any points.\\ \hline
    \end{tabular}
    \caption[Different types of objects]{Different types of objects}
    \label{tab:objects2}
\end{table}  
...
\subsubsection{Mechanics} 
Progression game. Solve a story. Completes each chapter or level by solving puzzles. Player has to finish different chapters to proceed to the next level. The levels are building on each other. Litt usikker på om dette er faktisk det!
...
\centering
    \begin{tabular}{|l|l|}
       ...
       {Music} & calm, classical music that changes to the speed \\ & of the game and to certain happenings. \\ \hline
       \emph{Vocalization} & the character will not have its own voice \\ \hline
       \emph{Sound effects} &   apples making a "pling"-sound when getting picked  \\ \hline
...
\centering
    \begin{tabular}{|l|l|}
      ...
      \emph {Perspective} & Third-person \\ \hline
       \emph{Dimension} &  3-dimensional and isometric \\ \hline
	       \emph{Exploration} &  A mix between forced and desired pace. The pace \\ & is forced because the apples appears at random times. \\ & At the same time the pace is desired because the player \\ & can self choose when he or she wants to stretch \\ & to gather the apple. However, this will also \\ & mean that the player chooses to lose an apple. \\ \hline
	       \emph{Time} & Real-time. However, the time of clock will not be shown. \\ & There will be a clock that counts from 2 \\ &minutes and down to 0 minutes. \\ \hline
